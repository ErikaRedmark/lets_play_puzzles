# Taken from http://garethrees.org/2015/11/09/exact-cover/ pretty much
# whole-cloth. Only modified a few things to account for the fact that
# this is intended specifically to draw the solution for Jewels of the Oracle,
# Assembly of the Planners

from svgwrite import Drawing
from itertools import product

# Uses the special identifiers for the Jewels puzzle specifically
...
COLOURS = dict(F_std="#EEAAAA", F_ref="#DDBB99", L="#CCCC88",
               P="#BBDD99", N_ref="#AAEEAA", T="#99DDBB",
               U="#88CCCC", V="#99BBDD", W="#AAAAEE",
               N_std_1="#BB99DD", N_std_2="#CC88CC", Z="#DD99BB",
               wall="#111111", empty="#CCCCCC")

_EMPTY = ' '


def draw_solution(solutions, filename, columns=1, size=25, padding=5,
                  colour=COLOURS.get, stroke_colour="black",
                  stroke_width=3):
    ...
    drawing_size = (padding + (columns * (w + 1) - 1) * size,
                    padding + (rows    * (h + 1) - 1) * size)
